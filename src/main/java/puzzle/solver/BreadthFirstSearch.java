 ...
 * @param <S> represents the states of the puzzle
 */
public class BreadthFirstSearch<T, S extends State<T, S>> {
     ...
     */
    public Optional<Node<T, S>> solve(S state) {
        Deque<Node<T, S>> open = new LinkedList<>();
        var seen = new HashSet<Node<T, S>>();
     ...
     * or an empty {@code Optional} if no solution is found
     */
    public Optional<Node<T, S>> solveAndPrintSolution(S state) {
    ...
    private void printPathTo(Node<T, S> node) {
